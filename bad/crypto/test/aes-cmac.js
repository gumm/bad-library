  ...
  describe('RFC4493 test vectors', function() {
    ...
    describe('generateSubkeys(key)', function() {
      it('creates the correct subkeys', function() {
        ...
        var result = bad.AesCmac.generateSubkeys(key);
        assert.deepEqual(result, expected);
      });
    });

    describe('aesCmac(key, message)', function() {
      it('generates the authentiation code for an empty input (length zero)',
        function() {
          var result = bad.AesCmac.aesCmac(key, new Buffer('', 'hex'));
          assert.equal(
            result.toString('hex'),
      ...
      it('generates the authentication code for a 16 octet input', function() {
        var result = bad.AesCmac.aesCmac(
          key, new Buffer('6bc1bee22e409f96e93d7e117393172a', 'hex'));
        assert.equal(
          result.toString('hex'),
      ...
      it('generates the authentication code for a 40 octet input', function() {
        var input = '6bc1bee22e409f96e93d7e117393172aae2d8a571e03ac9c9e' +
          'b76fac45af8e5130c81c46a35ce411';
        var result = bad.AesCmac.aesCmac(key, new Buffer(input, 'hex'));
        assert.equal(
          result.toString('hex'),
      ...
      it('generates the authentication code for a 64 octet input', function() {
        var input = '6bc1bee22e409f96e93d7e117393172aae2d8a571e03ac9c9eb76fac' +
          '45af8e5130c81c46a35ce411e5fbc1191a0a52eff69f2445df4f9b17ad2b417be6' +
          '6c3710';
        var result = bad.AesCmac.aesCmac(key, new Buffer(input, 'hex'));
        assert.equal(
          result.toString('hex'),
