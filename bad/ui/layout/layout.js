...
bad.ui.Layout.prototype.createDom = function() {
  ...
  this.setElementInternal(dom.createDom(
      ...
      {'id': this.getId(), 'class': bad.CssPrefix.LAYOUT}));
   ...
   * @type {string}
   */
  const myClassPrefix = this.getCellClassPrefix_();

  // Create the cells.
  Object.values(this.cell_).forEach(cell => {
    const className = bad.ui.Layout.CssClassMap.LAYOUT_CELL + ' ' +
  ...
  // Create the draggers.
  Object.values(this.dragger_).forEach(dragger => {
    dragger.element = dom.createDom(goog.dom.TagName.DIV, {
      'class': bad.ui.Layout.CssClassMap.DRAG_HANDLE + ' ' +
          bad.ui.Layout.CssClassMap.DRAG_HAND_FRAGMENT + dragger.dragClass +
          ' ' + this.getOrientClassName_() + ' ' + this.getGrabberClass_(),
      'style': this.checkOrient_('width') + ': ' + this.draggerThickness_ + 'px'
    });
...
bad.ui.Layout.prototype.enterDocument = function() {
  ...
  const target = /** @type {!Node} */ (this.getTarget());
  ...
  this.getHandler()
      .listen(
          this.dragger_.AB.dragger, goog.fx.Dragger.EventType.START,
          this.onDragStart_)
      .listen(
          this.dragger_.AB.dragger, goog.fx.Dragger.EventType.DRAG,
          goog.bind(this.onDrag_, this, this.dragger_.AB))
      .listen(
          this.dragger_.AB.dragger, goog.fx.Dragger.EventType.END,
          this.onABDragEnd_)
      .listen(
          this.dragger_.BC.dragger, goog.fx.Dragger.EventType.START,
          this.onDragStart_)
      .listen(
          this.dragger_.BC.dragger, goog.fx.Dragger.EventType.DRAG,
          goog.bind(this.onDrag_, this, this.dragger_.BC))
      .listen(
          this.dragger_.BC.dragger, goog.fx.Dragger.EventType.END,
          this.onBCDragEnd_)
      .listen(
  ...
  this.forEachChild(child => child.render());
...
bad.ui.Layout.prototype.updateChildSizes_ = function(e) {
  ...
  this.forEachChild(child => {
    const parentCell = this.getCellByName(child.getTargetCellName());
...
bad.ui.Layout.prototype.exitDocument = function() {
  ...
  Object.values(this.dragger_).forEach(item => item.dragger.dispose());
...
bad.ui.Layout.prototype.disposeInternal = function() {
  ...
  Object.values(this.innerLayout_).forEach(layout => layout.disposeInternal());
...
bad.ui.Layout.prototype.setLeft_ = function(rect, value) {
  this.orientation_ === bad.ui.Layout.Orientation.VERTICAL ? rect.top = value :
                                                             rect.left = value;
};
...
bad.ui.Layout.prototype.getLeft_ = function(rect) {
  return this.orientation_ === bad.ui.Layout.Orientation.VERTICAL ? rect.top :
                                                                    rect.left;
};
...
bad.ui.Layout.prototype.getHeight_ = function(rect) {
  return this.orientation_ === bad.ui.Layout.Orientation.VERTICAL ? rect.width :
                                                                    rect.height;
};
...
bad.ui.Layout.prototype.pullNests_ = function() {
  Object.values(this.innerLayout_).forEach(layout => {
    ...
    const nestId = '$' + layout.getTargetCellName();
    Object.entries(layoutNests).forEach(([key, nest]) => {
...
bad.ui.Layout.prototype.setInnerLayout = function(
  ...
  const id = Math.floor(Math.random() * 2147483648).toString(36);
  const cell = this.getCellByName(targetName);
  if (cell) {
    cell.innerLayoutId = id;
  }

  this.innerLayout_[id] = new bad.ui.Layout(id, names, orientation);
  const layout = this.innerLayout_[id];
  ...
  layout.setCellClassPrefix_(this.getCellClassPrefix_() + '-' + targetName);
  return layout;
};
...
bad.ui.Layout.prototype.createInnerLayout_ = function() {
  Object.values(this.innerLayout_).forEach(layout => {
    const parentCell = this.getCellByName(layout.getTargetCellName());
    /**
...
bad.ui.Layout.prototype.getNestElement = function(...var_args) {
  ...
  return nest ? nest.element : undefined;
};
...
bad.ui.Layout.prototype.setDraggerABLimits_ = function(value) {
  const rect = this.getDraggerLimitsRect_(this.dragger_.AB);

  this.setLeft_(rect, this.getMinimumSize_(this.cell_.A));
  this.setWidth_(
      rect, value - this.getLeft_(rect) - this.getDraggerThickness());
...
bad.ui.Layout.prototype.setDraggerBCLimits_ = function(max) {
  const draggerThickness = this.getDraggerThickness();
  const rect = this.getDraggerLimitsRect_(this.dragger_.BC);
...
bad.ui.Layout.prototype.setSize = function(givenWidth, givenHeight) {
  ...
  const newRect = new goog.math.Rect(
      margin.left, margin.top, givenWidth - margin.left - margin.right,
  ...
  if (this.isInDocument()) {
    ...
    this.updatePositionAndSize_(
        /** @type {!Element} */ (this.getElement()), newRect);

    const bCRect = this.dragger_.BC.rect;
    const cRectWidth =
        this.getWidth_(this.cell_.C.rect) + this.getDraggerThickness();
...
bad.ui.Layout.prototype.createRects_ = function() {
  const initTargetSize = goog.style.getBorderBoxSize(this.getTarget());
  const margin = this.getMargin();
  ...
  const left = 0;
  const top = 0;
  const iniA = this.cell_.A.initSize;
  const iniC = this.cell_.C.initSize;

  const rootHeight = this.getHeight_(this.componentBoxSize_);
  const rootWidth = this.getWidth_(this.componentBoxSize_);
  const thickness = this.draggerThickness_;
 ...
 * @private
 */
bad.ui.Layout.prototype.updatePositionAndSize_ = function(
    element, rect, opt_nestId) {
  ...
  if (opt_nestId) {
    this.dispatchEvent(new bad.ui.Layout.Event(
        bad.ui.Layout.EventType['SIZE_CHANGED_' + opt_nestId], this,
        {element: element, rect: rect, nestId: opt_nestId}));
  }
};
...
bad.ui.Layout.prototype.updateVariableSizes_ = function() {
  ...
  // and so on.
  const aEdgeAB = this.getLeft_(this.dragger_.AB.rect);
  const aBEdgeB = aEdgeAB + this.draggerThickness_;
  const bEdgeBC = this.getLeft_(this.dragger_.BC.rect);
  const bCEdgeC = bEdgeBC + this.draggerThickness_;
  ...
  this.setWidth_(
      this.cell_.C.rect, this.getWidth_(this.componentBoxSize_) - bCEdgeC);
  ...
  this.updatePositionAndSize_(this.cell_.A.element, this.cell_.A.rect);
  this.updatePositionAndSize_(this.dragger_.AB.element, this.dragger_.AB.rect);
  this.updatePositionAndSize_(this.cell_.B.element, this.cell_.B.rect);
  this.updatePositionAndSize_(this.dragger_.BC.element, this.dragger_.BC.rect);
  this.updatePositionAndSize_(this.cell_.C.element, this.cell_.C.rect);
...
bad.ui.Layout.prototype.onPanelMinimize_ = function(e) {
  ...
  //  const nestId = e.target.getNestId();
...
bad.ui.Layout.prototype.onDrag_ = function(dragger, e) {
  const pos = this.flipLeft_() === 'left' ? e.left : e.top;
...
bad.ui.Layout.prototype.onABDragEnd_ = function() {
   ...
   * the panel.
   */
  const result = this.getLeft_(this.dragger_.AB.rect);
  const minimum = this.getMinimumSize_(this.cell_.A);
...
bad.ui.Layout.prototype.onBCDragEnd_ = function() {
   ...
   * then set the memory.
   */
  const result = this.getLeft_(this.dragger_.BC.rect);
  const minimum = this.getMinimumSize_(this.cell_.C);
...
bad.ui.Layout.prototype.matchSizeToViewport = function() {
  const viewPortSize = this.viewportSizeMonitor_.getSize();
  const targetElementRect = goog.style.getBounds(this.getTarget());
  let width = targetElementRect.width;
  let height = targetElementRect.height;
...
bad.ui.Layout.prototype.addInteractionA_ = function(compA, compC, dragAB) {
  ...
  compA.getLayout = () => this;

  /**
   * Helper function to hide the A cell.
   * @param {?Function=} opt_callback
   */
  compA.hide = opt_callback => {
   ...
   */
  compA.close = (opt_callback, opt_animate) => {
    this.memoryAB_ = Math.max(
        this.getLeft_(dragAB.rect), this.getMinimumSize_(this.cell_.A));
    if (opt_animate) {
   ...
   * @param {number=} opt_pixels
   */
  compA.slideOpen = (opt_percentage, opt_pixels, opt_callback) => {
   ...
   * @param {?Function=} opt_callback
   */
  compA.slideClosed = (opt_callback) => {
    const size = this.getWidth_(compA.rect);
    if (size > 0) {
      const animate = true;
   ...
   * @param {number=} opt_pixels
   */
  compA.slideTo = (opt_callback, opt_percentage, opt_pixels) => {
    const animate = true;
    compA.show(opt_callback, opt_percentage, opt_pixels, animate);
  };
   ...
   */
  compA.toggle = (opt_callback, opt_perc, opt_pix) => {
    const size = this.getWidth_(compA.rect);
    const animate = true;
    if (size <= 0) {
   ...
   */
  compA.isOpen = () => this.getWidth_(compA.rect);
};
...
bad.ui.Layout.prototype.addInteractionC_ = function(compA, compC, dragBC) {
  ...
  compC.getLayout = () => this;

  /**
   * Helper function to hide the C cell.
   * @param {?Function=} opt_callback
   */
  compC.hide = (opt_callback) => {
   ...
   * @param {boolean=} opt_animate
   */
  compC.close = (opt_callback, opt_animate) => {
    const result = this.getLeft_(dragBC.rect);
    const fullSize = this.getWidth_(this.componentBoxSize_);
    const draggerThickness = this.getDraggerThickness();
    this.memoryBC_ = fullSize - result;
    const closeTo = fullSize - draggerThickness;
   ...
   * @param {number=} opt_pixels
   */
  compC.slideTo = (opt_callback, opt_percentage, opt_pixels) => {
    const animate = true;
    compC.show(opt_callback, opt_percentage, opt_pixels, animate);
  };
   ...
   * @param {?Function=} opt_callback
   */
  compC.slideClosed = (opt_callback) => {
    const size = this.getWidth_(compC.rect);
    if (size > 0) {
      const animate = true;
   ...
   * @param {number=} opt_pix
   */
  compC.toggle = (opt_callback, opt_perc, opt_pix) => {
    const size = this.getWidth_(compC.rect);
    const animate = true;
    if (size <= 0) {
   ...
   * Helper function to check if the cell is open.
   */
  compC.isOpen = () => this.getWidth_(compC.rect);
};
...
bad.ui.Layout.prototype.doAnimate_ = function(dragger, end, opt_callback) {
  ...
  this.getHandler()
      .listen(anim, goog.fx.Animation.EventType.ANIMATE, onAnimate)
      .listen(anim, goog.fx.Transition.EventType.END, onEnd);
  anim.play();
};
